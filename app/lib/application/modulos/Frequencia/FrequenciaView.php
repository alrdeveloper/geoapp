...
class FrequenciaView extends AppView {
    ...
    public function principal($obj, $post, $pg) {
    	$this->nav->Adicionar("Frequencia - Principal");
    	$this->assign("Nav", $this->nav->Gerar());
        
    	$this->assign("Obj", $obj);
        parent::principal($post, "Frequencia", $pg);
    }
    ...
    public function gerenciar($obj, $result, $select) {
    	$this->nav->Adicionar("Frequencia - Principal", "/Frequencia/principal");
    	if (empty($obj->getId())) {
        ...
        parent::gerenciar($obj, "Frequencia", $result);
    }

...
class AreaFuncionarioLigacaoController extends AppController
{
	private $view, $sql;

	public function __construct()
	{
		$this->view = new AreaFuncionarioLigacaoView("AreaFuncionarioLigacao");
		$this->sql = new AppInstructionSQL("area_funcionario_ligacao");
	}

	public function deleteAction($param)
	{
		$entidade = new AreaFuncionarioLigacao();
		$result = parent::deleteAction($entidade, $this->sql, $param);
		if (! is_array($result))
		{
			$this->redirect("/AreaFuncionarioLigacao/principal");
		}
	}
	...
	public function principalAction($param, $get, $post)
	{
		...
		$path = "/AreaFuncionarioLigacao/principal/";
		if (empty($post["param"]))
		{

...
class AreaFuncionarioLigacao extends AppEntidade implements IEntidade {

    private $area_funcionario_ligacao, $preferencia;
    ...
    public function getAreaFuncionarioLigacao() {
        return $this->area_funcionario_ligacao;
    }

    public function setAreaFuncionarioLigacao($area_funcionario_ligacao) {
        $this->area_funcionario_ligacao = $area_funcionario_ligacao;
    }
    ...
    public function validaInsert() {
        $msg = array();
        if (empty($this->area_funcionario_ligacao)) {
            $msg[] = "O campo AreaFuncionarioLigacao � obrigat�rio";
        }
        return $msg;
    }
    ...
    public function listData($cmd, $values) {
        ...
        if (!empty($resultSet)) {
            for ($i = 0; $i < count($resultSet); $i++) {
                $obj[$i] = new AreaFuncionarioLigacao("");
                $obj[$i]->setId($resultSet[$i]["id"]);
                $obj[$i]->setIdentificador($resultSet[$i]["identificador"]);
                $obj[$i]->setAreaFuncionarioLigacao($resultSet[$i]["area_funcionario_ligacao"]);
    ...
    public function loadData($cmd, $values) {
    	...
    	if (!empty($resultSet)) {
   			...
   			$this->setAreaFuncionarioLigacao($resultSet["area_funcionario_ligacao"]);

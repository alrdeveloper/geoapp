 ...
 */
namespace application\modulos\AreaFuncionario;
...
class AreaFuncionarioController extends AppController
{
	private $view, $sql;

	public function __construct()
	{
		$this->view = new AreaFuncionarioView("AreaFuncionario");
		$this->sql = new AppInstructionSQL("area_funcionario");
	}

	public function deleteAction($param)
	{
		$entidade = new AreaFuncionario();
		$result = parent::deleteAction($entidade, $this->sql, $param);
		if (! is_array($result))
		{
			$this->redirect("/AreaFuncionario/principal");
		}
	}
	...
	public function gerenciarAction($param, $get, $post, $file)
	{
		...
		$entidade = new AreaFuncionario();
		$listEntidade = $entidade->returnAreaFuncionariosForSelect();		
		if (! empty($param))
		{
	...
	public function principalAction($param, $get, $post)
	{
		...
		$path = "/AreaFuncionario/principal/";
		if (empty($post["param"]))
		{

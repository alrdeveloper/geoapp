...
class Documento extends AppEntidade implements IEntidade {

    private $documento, $preferencia;
    ...
    public function validaInsert() {
        ...
        if (empty($this->documento)) {
            $msg[] = "O campo Documento � obrigat�rio";
        }
        return $msg;
    }
    ...
    public function listData($cmd, $values) {
        ...
        if (!empty($resultSet)) {
            for ($i = 0; $i < count($resultSet); $i++) {
                ...
                $obj[$i]->setDocumento($resultSet[$i]["documento"]);
    ...
    public function loadData($cmd, $values) {
    	...
    	if (!empty($resultSet)) {
   			...
   			$this->setDocumento($resultSet["documento"]);

...
class ModeloView extends AppView {
    ...
    public function principal($obj, $post, $pg) {
    	$this->nav->Adicionar("Modelo - Principal");
    	$this->assign("Nav", $this->nav->Gerar());
        
    	$this->assign("Obj", $obj);
        parent::principal($post, "Modelo", $pg);
    }
    ...
    public function gerenciar($obj, $result, $select) {
    	$this->nav->Adicionar("Modelo - Principal", "/Modelo/principal");
    	if (empty($obj->getId())) {
        ...
        parent::gerenciar($obj, "Modelo", $result);
    }

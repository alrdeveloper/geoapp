...
namespace application\modulos\Parada;

use application\lib\AppView;

class ParadaView extends AppView {
    ...
    public function principal($obj, $post, $pg) {
    	$this->nav->Adicionar("Parada - Principal");
    	$this->assign("Nav", $this->nav->Gerar());
        
    	$this->assign("Obj", $obj);
        parent::principal($post, "Parada", $pg);
    }
    ...
    public function gerenciar($obj, $result, $select) {
    	$this->nav->Adicionar("Parada - Principal", "/Parada/principal");
    	if (empty($obj->getId())) {
        ...
        parent::gerenciar($obj, "Parada", $result);
    }
